...
;var VisualSearch = (function($, _, Backbone){
	...
	$.fn.getCursorPosition = function() {
		...
		} else if (input && $(input).is(':visible')) { // Firefox/Safari
			try { if (input.selectionStart != null) { position = input.selectionStart; } }
			catch (e) {
				if (!(e instanceof window.DOMException && e.name === "InvalidStateError")) {
					throw e;
				}
				return null;
			}
		...
		initialize : function(){
			...
			this.$el.on("keypress keyup keydown", "input", function(e){
				var container = self.$(".VS-search-inner"),
					containerwidth = container.width()-parseInt(container.css('margin-left'), 10),
					input = $(this),
					shadow = $("<span>").css({
								position: 'absolute',
								top: -9999,
			...
			var self = this,
				editNext = function(){
					...
					if( editing===2 ){
						...
						// temporary kludge -- todo: remove this blur trigger and submit new parameters properly
						// this function seems incorrect, at least according to my expected interaction patterns
						// $(e.target).blur();
	...
	VS.ParameterView = Backbone.View.extend({
		...
		render : function() {
			...
			var operators = parameters.operators[
				parameters.key.indexOf(this.model.get('key'))],
				defaultOperator = operators ? operators[0] : "==";
			this.operator = {
				dom: $("<input/>").attr({
					autocomplete: "off",
					name:         "operator",
				...
				autocomplete: {
					source: function(req, res){
						var key = self.model.get('key'),
							...
							operators = parameters.operators[i];
						res(operators ? operators : VS.defaultOperators);
					}
				}
			};
			this.value = {
			...
			if( !this.model.has('key') || this.model.get('editing')===0 ){
				...
				if( !this.model.has('operator') || this.model.get('editing')===1 ){
					if( operators != null && operators.length <= 1 ){
						this.operator.dom[0].value = operators[0];
						// this.operator.dom.blur();
						this.inputBlurred({ target: this.operator.dom[0] })
					} else {
						this.autocomplete(
		...
		inputBlurred: function(e){
			...
			var update, val = input.val();

			if( editing == 1 ){
				var parameters = this.model.collection.parameters,
		...
		click: function(e){
			...
			if(clicked.count>0 || $(e.target).is("div.value")){
				this.dblclick(e);
			}else {
				//Single Click
				var self = this;
		...
		autocomplete: function(target, options){
			target.autocomplete(_.extend({
				// autoFocus: true, // TODO: fix tabbing when this is enabled, and enable it.
				minLength : 0,
				delay     : 0,
			...
			}, options)).autocomplete('widget').addClass('VS-interface');
			return target;
		},
		keydown : function(e) {
	...
	VS.Parameter = Backbone.Model.extend({
		...
		setType: function(){
			...
			if(i!==-1){
				this.set({
					"type": (_ref = collection.parameters.type) != null ? _ref[i] : undefined,
					"placeholder": collection.parameters.placeholder[i],
					"min": (_ref = collection.parameters.min) != null ? _ref[i] : undefined,
					"max": (_ref = collection.parameters.max) != null ? _ref[i] : undefined,
				});
			}
		}
	});
